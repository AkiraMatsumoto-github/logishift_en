...
> 4.  **Repository Integrity**: `logishift` (本家) と `logishift_en` (英語版) は完全に別のリポジトリである。`push` や `pull` を行う前には、必ず `git remote -v` を実行し、現在操作しようとしているリモート先が意図したものか（`origin` がどちらを向いているか）を指差し確認すること。**絶対に間違ったリポジトリにコミットしないこと。**
