...
<main id="primary" class="site-main">
	...
	<section class="archive-hero">
		<div class="container">
			<h1 class="archive-title"><?php esc_html_e( 'Popular Articles Ranking', 'logishift' ); ?></h1>
			<div class="archive-description">
				<?php esc_html_e( 'Presenting the most read articles on LogiShift in a ranking format.', 'logishift' ); ?>
			</div>
		</div>
	</section>
		...
		<div class="container">
			...
			<div class="region-filter-tabs ranking-tabs" style="justify-content: center; margin-bottom: 40px;">
				<button class="region-tab active" data-period="7">
					<?php esc_html_e( '7 Days', 'logishift' ); ?>
				</button>
				<button class="region-tab" data-period="14">
					<?php esc_html_e( '14 Days', 'logishift' ); ?>
				</button>
				<button class="region-tab" data-period="30">
					<?php esc_html_e( '30 Days', 'logishift' ); ?>
				</button>
				<button class="region-tab" data-period="60">
					<?php esc_html_e( '60 Days', 'logishift' ); ?>
				</button>
			</div>
			...
			foreach ( $periods as $index => $days ) :
				...
				$start_date = date_i18n( 'M j, Y', strtotime( "-$days days" ) );
				$end_date   = date_i18n( 'M j, Y' );
				?>
				<div class="ranking-block" id="ranking-block-<?php echo esc_attr( $days ); ?>" style="display: <?php echo $display_style; ?>;">
					...
					<p class="ranking-period-info" style="text-align: center; color: #888; margin-bottom: 20px; font-size: 0.9em;">
						<?php printf( esc_html__( 'Period: %s - %s', 'logishift' ), $start_date, $end_date ); ?>
					</p>
					...
					<div class="featured-grid">
						...
						if ( function_exists( 'logishift_get_popular_posts' ) ) {
							...
							if ( ! empty( $popular_posts ) ) {
								...
								echo '<p style="text-align:center; width:100%;">' . esc_html__( 'No data available.', 'logishift' ) . '</p>';
							}
						}
						?>
					</div>
