...
### Q. 投稿時に `403 Forbidden` エラーが出る
**原因:** Access Token の権限が "Read" のままになっている可能性が高いです。
**対処法:**
1. Developer Portal で App permissions が **"Read and Write"** になっているか再確認してください。
2. **重要:** 権限設定を変更した**後**に、必ず **Access Token and Secret を再生成 (Regenerate)** してください。設定を変えただけではトークンの中身は更新されません。
3. 再生成した新しいトークンを `.env` に上書きしてください。
